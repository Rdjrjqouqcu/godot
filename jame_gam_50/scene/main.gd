...
func redraw_text() -> void:
	...
	text = ""
	var min:int = int(time_elapsed_ms / 60 / 100)
	var sec:int = int(time_elapsed_ms / 100) % 60
	var ms:int = int(time_elapsed_ms) % 100
	time_display.text = "%02d:%02d.%02d" % [min, sec, ms]
